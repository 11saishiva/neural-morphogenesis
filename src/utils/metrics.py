...
def extract_local_patches(state, patch_size=5):
    """
    Safe patch extractor — NO as_strided, NO views.
    Always returns contiguous patches.
    ...
    # pad to allow extraction at boundaries
    ...
    for y in range(H):
        for x in range(W):
            patch = padded[:, :, y:y+p, x:x+p]
            # FORCE contiguous memory copy
            patches.append(patch.clone().contiguous())
